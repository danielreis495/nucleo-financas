  ...
  return (
    <section className="mt-6 rounded-xl bg-elevated p-4 shadow-[var(--shadow-border)]">
      ...
      {accounts.length ? (
        ...
        <p className="mt-4 rounded-lg bg-surface p-3 text-sm text-muted">Nenhuma conta cadastrada ainda.</p>
      )}

      {open ? (
        <div className="mt-4 rounded-lg bg-surface p-3">
          <h3 className="text-sm font-medium">Nova conta</h3>
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Nome (ex.: Conta principal)" className="mt-3 h-11 w-full rounded-md bg-elevated px-3 text-sm shadow-[var(--shadow-border)] outline-none focus:outline-2 focus:outline-primary" />
          <input value={institution} onChange={(e) => setInstitution(e.target.value)} placeholder="Banco / instituição" className="mt-2 h-11 w-full rounded-md bg-elevated px-3 text-sm shadow-[var(--shadow-border)] outline-none focus:outline-2 focus:outline-primary" />
          ...
          <div className="mt-3 flex gap-2">
            <Button className="flex-1" disabled={!name.trim()} onClick={() => {
              ...
              addAccount({ name, institution, type, openingBalance: Number.isFinite(value) ? value : 0 });
              setName(""); setInstitution(""); setBalance(""); setType("checking"); setOpen(false);
