...
export const useFinanceStore = create<FinanceState & FinanceActions>()(
  persist(
    (set, get) => ({
      ...
      resetDemo: () => set({
        ...createSeedState(), hydrated: true, viewMonth: monthKey(new Date()), geminiKey: get().geminiKey, customCategories: get().customCategories,
      }),
      clearAll: () => set({
        ...emptyState(), hydrated: true, viewMonth: monthKey(new Date()), geminiKey: get().geminiKey, customCategories: get().customCategories,
      }),
      setHouseholdName: (householdName) => set({ householdName }),
      addPerson: ({ name, role, color }) => set({ people: [...get().people, { id: uid(), name, role, color, monthlyBudget: null }] }),
      updatePerson: (id, patch) => set({ people: get().people.map((p) => (p.id === id ? { ...p, ...patch } : p)) }),
      removePerson: (id) => {
        ...
        set({
          ...
          transactions: transactions.map((t) => (t.personId === id ? { ...t, personId: fallback } : t)),
      ...
      addAccount: ({ name, institution, type, openingBalance }) => {
        ...
        const account: Account = {
          id: uid(), name: trimmedName, institution: institution.trim(), type,
          openingBalance: Number.isFinite(openingBalance) ? openingBalance : 0,
          createdAt: new Date().toISOString(), active: true,
        };
        set({ accounts: [account, ...get().accounts], demo: false });
      },
      updateAccount: (id, patch) => set({ accounts: get().accounts.map((a) => (a.id === id ? { ...a, ...patch } : a)) }),
      removeAccount: (id) => {
      ...
      addQuickExpense: ({ amount, category, personId, description, accountId }) => {
        const date = todayIso();
        const t: Transaction = {
          id: uid(), date, description: description || "Gasto rápido", merchant: description || "Lançamento",
          amount, type: "expense", status: "posted", category, personId, accountId: accountId ?? null,
          split: null, installmentId: null, installmentIndex: null, installmentTotal: null,
      ...
      addCustomCategory: ({ label, group }) => {
        ...
        let id = trimmed.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 32);
      ...
      importExtracted: (items, source) => {
        ...
        for (const item of selected) {
          if (item.installment && item.installment.total > 1) {
            ...
            newPlans.push({
              id: planId, title: item.description || item.merchant, merchant: item.merchant, kind: item.installment.kind,
              installmentAmount: item.amount, totalCount: item.installment.total, startDate: isoDate(start), personId: item.personId,
              category: item.category, account: "",
            });
            newTx.push(...expandNewPlan({
              id: planId, title: item.description || item.merchant, merchant: item.merchant, kind: item.installment.kind,
              installmentAmount: item.amount, totalCount: item.installment.total, startDate: isoDate(start), personId: item.personId,
              category: item.category, currentIndex: item.installment.current,
            }));
          } else {
            newTx.push({
              id: uid(), date: item.date, description: item.description, merchant: item.merchant, amount: item.amount,
              type: item.type, status: "posted", category: item.category, personId: item.personId, split: null,
              installmentId: null, installmentIndex: null, installmentTotal: null, source, createdAt: new Date().toISOString(),
            });
          }
        }
        set({ transactions: [...newTx, ...get().transactions], plans: [...newPlans, ...get().plans], demo: false });
      },
      addInstallmentPlan: (input) => {
        const id = uid();
        const plan = {
          id, title: input.title, merchant: input.merchant, kind: input.kind, installmentAmount: input.installmentAmount,
          totalCount: input.totalCount, startDate: input.startDate, personId: input.personId, category: input.category,
          account: input.account ?? "",
        };
        const txs = expandNewPlan({ ...input, id });
        set({ plans: [plan, ...get().plans], transactions: [...txs, ...get().transactions], demo: false });
      },
      setAdvice: (advice) => set({ advice }),
      advanceDueInstallments: () => {
        const today = todayIso();
        const transactions = get().transactions.map((t) => {
          if (t.status === "scheduled" && t.installmentId && t.date <= today) return { ...t, status: "posted" as const };
          return t;
        });
        set({ transactions });
      },
    }),
    {
      ...
      merge: (persisted, current) => {
        ...
        return { ...current, ...saved, accounts: Array.isArray(saved.accounts) ? saved.accounts : [] };
      },
      partialize: (s) => ({
        householdName: s.householdName, people: s.people, accounts: s.accounts, transactions: s.transactions,
        plans: s.plans, budgets: s.budgets, customCategories: s.customCategories, advice: s.advice,
        demo: s.demo, geminiKey: s.geminiKey,
      }),
    },
  ),
);
