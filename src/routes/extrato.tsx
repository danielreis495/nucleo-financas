...
function ExtratoPage() {
  ...
  function handleDelete(tx: Transaction) {
    ...
    toast("Lançamento apagado", {
      duration: 7000,
      action: { label: "Desfazer", onClick: () => restore(tx) },
    });
  }
    ...
    <main className="flex flex-col pb-6">
      ...
      {catLabel ? (
        <div className="mx-5 mb-3 rounded-xl bg-elevated px-4 py-3 shadow-[var(--shadow-border)]">
          ...
          <p className="mt-1 text-sm text-muted">{rows.length} lançamento{rows.length === 1 ? "" : "s"} · {formatBRL(Math.abs(total))}</p>
        </div>
      ) : null}
      <div className="flex gap-2 overflow-x-auto px-5 pb-2">
        ...
        {state.people.map((p) => <FilterChip key={p.id} active={personId === p.id} onClick={() => setPersonId(p.id)}>{p.name}</FilterChip>)}
      </div>
      <div className="flex gap-2 overflow-x-auto px-5 pb-4">
        <FilterChip active={category === "all"} onClick={() => setCategory("all")}>Categorias</FilterChip>
        {filterCats.map((c) => <FilterChip key={c.id} active={category === c.id} onClick={() => setCategory(c.id)}>{c.label}</FilterChip>)}
      </div>
      {groups.length === 0 ? (
        <p className="px-5 text-sm text-muted">Nenhum lançamento neste recorte.</p>
      ) : groups.map(([date, list]) => (
        <section key={date} className="px-5 pb-4">
          <h2 className="mb-2 text-xs font-medium tracking-wide text-muted uppercase">{formatLongDate(date)}</h2>
              ...
              return (
                <li key={t.id} className={i > 0 ? "border-t border-line" : ""}>
                  <SwipeRow onDelete={() => handleDelete(t)}>
                    <button className="flex w-full items-center gap-3 bg-elevated px-4 py-3 text-left" onClick={() => setEditing(t)}>
                      ...
                      <span className="min-w-0 flex-1">
                        ...
                        <span className="block text-xs text-muted">
                          {categoryLabel(t.category, custom)}{account ? ` · ${account.name}` : " · sem conta"}{t.installmentIndex ? ` · ${t.installmentIndex}/${t.installmentTotal}` : ""}{scheduled ? " · agendado" : ""}
                        </span>
                      </span>
                      <span className={cn("font-display text-sm tabular-nums", t.type === "income" ? "text-income" : "text-fg", scheduled && "text-muted")}>{t.type === "income" ? "+" : "−"}{formatBRL(t.amount)}</span>
                    </button>
                  </SwipeRow>
                </li>
              );
      ...
      {editing && state.transactions.some((t) => t.id === editing.id) ? <TransactionEdit tx={editing} onClose={() => setEditing(null)} /> : null}
    </main>
  );
}

function FilterChip({ active, onClick, children }: { active: boolean; onClick: () => void; children: ReactNode }) {
  return <button onClick={onClick} className={cn("h-9 shrink-0 rounded-full px-3 text-xs font-medium", active ? "bg-primary text-primary-fg" : "bg-line text-fg")}>{children}</button>;
}
