  ...
  return (
    <main className="flex flex-col px-5 pb-8 pt-5">
      <div className="flex items-end justify-between gap-3">
        <div>
          <p className="text-[11px] font-medium uppercase tracking-[0.14em] text-muted">Planejamento</p>
      ...
      <p className="mt-2 text-sm leading-relaxed text-muted">
        Acompanhe o que ainda falta pagar e confirme pagamentos apenas quando eles realmente acontecerem.
      </p>
      ...
      <section className="mt-4 overflow-hidden rounded-2xl bg-primary text-primary-fg shadow-[var(--shadow-border)]">
        <div className="px-5 pb-4 pt-5">
          ...
          <p className="mt-1 text-xs text-primary-fg/70">
            {active.length} compromisso{active.length === 1 ? "" : "s"} ativo{active.length === 1 ? "" : "s"}
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 divide-x divide-primary-fg/10 border-t border-primary-fg/10 bg-primary-fg/[0.04]">
          ...
          <HeroMetric
            label="Próxima"
            value={nextPlan?.progress.next ? formatLongDate(nextPlan.progress.next.date) : "Nada pendente"}
          />
        </div>
      </section>
        ...
        <details className="group mt-4 rounded-xl bg-elevated shadow-[var(--shadow-border)]">
          <summary className="flex cursor-pointer list-none items-center justify-between px-4 py-3">
            <span>
              ...
              <span className="block text-xs text-muted">
                {finished.length} compromisso{finished.length === 1 ? "" : "s"} concluído{finished.length === 1 ? "" : "s"}
              </span>
            </span>
          ...
          <ul className="border-t border-line px-4">
            {finished.map(({ plan, progress }) => (
              <li key={plan.id} className="flex items-center justify-between gap-3 border-b border-line py-3 last:border-0">
                <div className="min-w-0">
                  <p className="truncate text-sm font-medium">{plan.title}</p>
                  <p className="text-xs text-muted">{progress.total}/{progress.total} parcelas</p>
                </div>
                <span className="shrink-0 rounded-full bg-primary-soft px-2 py-1 text-[10px] font-medium text-primary">
                  Quitado
                </span>
              </li>
            ))}
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 grid grid-cols-[7rem_1fr] items-end gap-3">
        <div>
          <label className="text-xs font-medium text-muted" htmlFor="installment-count">Parcelas</label>
          <input
        ...
        <p className="pb-2 text-sm text-muted">
          Total programado: <span className="font-medium text-fg">{formatBRL(amount * totalCount)}</span>
        </p>
      </div>
